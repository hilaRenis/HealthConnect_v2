...
async function getProducer() {
  ...
  try {
    ...
    console.error('[admin-service] Failed to establish Kafka connection, disabling publisher', error);
    kafkaDisabled = true;
    try {
      await producer.disconnect();
    } catch (disconnectErr) {
      console.error('[admin-service] Failed to disconnect Kafka producer cleanly', disconnectErr);
    }
    producer = null;
    return null;
  }
...
function sendAsync(producerInstance, payload) {
  producerInstance.send(payload).catch((error) => {
    console.error('[admin-service] Failed to publish Kafka event', payload.topic, error);
...
async function startConsumer({ groupId, topics, handleMessage }) {
  if (!kafka || kafkaDisabled) {
    console.warn('[admin-service] Kafka disabled, consumer not started');
    return null;
  }
  ...
  try {
    ...
    console.error('[admin-service] Failed to connect Kafka consumer, disabling consumer', error);
    kafkaDisabled = true;
    return null;
  }
  ...
  await consumer.run({
    eachMessage: async ({ topic, message }) => {
      ...
      try {
        ...
        console.error('[admin-service] Failed to handle Kafka message', topic, err);
      }
    },
  });
