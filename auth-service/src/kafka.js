...
async function getProducer() {
  ...
  try {
    ...
    console.error('[auth-service] Failed to establish Kafka connection, disabling publisher', error);
    kafkaDisabled = true;
    try {
      await producer.disconnect();
    } catch (disconnectErr) {
      console.error('[auth-service] Failed to disconnect Kafka producer cleanly', disconnectErr);
    }
    producer = null;
    return null;
  }
...
function sendAsync(producerInstance, payload) {
  producerInstance.send(payload).catch((error) => {
    console.error('[auth-service] Failed to publish Kafka event', payload.topic, error);
